...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hInstancePrev, LPSTR pCmdLine, int nCmdShow)
{
	...
	// ウィンドウの作成
	WHandle = CreateWindow(ClassName, "ポリゴンの描画", WIN_STYLE, CW_USEDEFAULT, CW_USEDEFAULT, 1000, 800, NULL, NULL, hInstance, NULL);
	...
	while (msg.message != WM_QUIT) {
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			// パラメータの計算
			XMVECTOR eye_pos = XMVectorSet(0.0f, 1.0f, -2.0f, 1.0f);        // 視点位置
			...
			XMMATRIX World = XMMatrixRotationY(x += 0.001);                 // z軸を回転軸とした回転行列
